...
**Request:**
```bash cURL
curl -X GET 'https://api.gameball.co/api/v4.0/integrations/customers/cust_12345/balance' \
...
**Request:**
```bash cURL
curl -X DELETE 'https://api.gameball.co/api/v4.0/integrations/transactions/hold/abc-123' \
  -H 'APIKey: YOUR_API_KEY' \
  -H 'SecretKey: YOUR_SECRET_KEY'
```
...
|------|-----|----------------|
| Show points balance | `GET /api/v4.0/integrations/customers/{customerId}/balance` | Call API when customer lands on checkout<br/>Display available points and monetary equivalent<br/>Add slider/input field for point selection |
| Apply points (Hold Points) | `POST /api/v4.0/integrations/transactions/hold` | Call API when customer clicks "Apply Points"<br/>Store the returned `holdReference`<br/>Show real-time discount calculation |
| Place order (Finalize Redemption) | `POST /api/v4.0/integrations/orders` | Call API when customer confirms order<br/>Pass the stored `holdReference` in redemption object<br/>Points are permanently deducted from customer balance |
| Handle order cancellation (Optional) | `DELETE /api/v4.0/integrations/transactions/hold/{holdReferenceId}` | Call API if customer cancels before timeout<br/>Use the original `holdReference`<br/>Points are returned to customer's available balance |
