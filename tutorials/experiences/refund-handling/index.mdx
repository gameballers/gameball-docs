...
flowchart TB
    ...
    B -->|Yes| C["🎯 Accurate reverse calculation"]
    B -->|No| D["⚠️ Cashback-only rollback"]
```
---
...
<ParamField path="transactionTime" type="string">
  Ensures proper audit tracking and timeline alignment. Must be in UTC (ISO 8601 format).
</ParamField>
...
```mermaid
flowchart LR
    A["🛍️ Bought: Running Shoes ($12.50)"] --> B["🎁 Earned: 12 pts"]
    B --> C["↩️ Refund Shoes"]
    C --> D["❌ Reverse 12 pts"]
```
...
```mermaid
flowchart TB
    A["🎮 Purchase: $25 Console"] --> B["Base Points: 25"]
    B --> C["Campaign Multiplier ×3"]
    C --> D["Earned: 75 pts"]
    D --> E["↩️ Refund Console"]
    E --> F["❌ Reverse 75 pts"]
```
...
```mermaid
flowchart LR
    A["Refund Request"] --> B["⚠️ No lineItems"]
...
```mermaid
flowchart TB
    A["🧘 Bought 3 Yoga Mats ($10 each)"] --> B["🌊 Double Points"]
    B --> C["Earned: 60 pts"]
    C --> D["↩️ Refund 2 Mats ($20)"]
    D --> E["❌ Reverse 40 pts"]
```
