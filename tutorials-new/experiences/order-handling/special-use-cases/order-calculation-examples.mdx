...
| `redeemedAmount` | Currency value of points redeemed through Gameball in this order | `redeemedAmount: 100` |
...
totalDiscount = Σ(lineItem.discount) + orderLevelDiscount + redeemedAmount + couponDiscount
...
<Info>
`redeemedAmount` reflects the currency value of the redeemed points. Customer earns points only on `totalPaid` (475).
</Info>
...
<Info>
All discounts (commercial + points + coupon) are summed in `totalDiscount`. `redeemedAmount` reflects the Gameball points portion only.
</Info>
...
<Warning>
Do not use `redeemedAmount` or the `redemption` object for external loyalty programs. Include the redeemed amount in `totalDiscount` only. Customer earns Gameball points on `totalPaid` (475).
</Warning>
...
- `redeemedAmount` (when Gameball points are redeemed — currency value of points used)
</Check>
