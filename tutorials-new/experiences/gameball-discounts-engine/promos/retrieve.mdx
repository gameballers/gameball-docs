---
title: "Display Promo Codes for Customers"
...
The [**Get Customer Coupons API**](https://gameball.mintlify.app/api-reference/customers/management/get-customer-coupons) returns all **promotions** assigned to a customer, including both active and expired ones.
...
- The [**Get Customer Coupons API**](https://gameball.mintlify.app/api-reference/customers/management/get-customer-coupons) returns all promotions, including expired or used ones.
- You must filter them using `isExpired`, `isActive`, and `isAvailableToUse`.
- Combine with `customerUsedCount` and `limitPerCustomer` to handle per-customer usage limits.
- Ideal for **custom UIs**, **mobile apps**, and **POS systems** showing personalized offers.
- Gameball’s **Promo Engine** keeps lifecycle tracking consistent across **Validate**, **Burn**, and **Release** flows.
