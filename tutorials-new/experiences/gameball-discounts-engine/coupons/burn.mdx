...
```mermaid
flowchart LR
    A["📋 Retrieve<br/>Coupons"] --> B["🧪 Validate<br/>Coupon"]
    B --> C["🔒 Lock<br/>Coupon"]
    C --> D["🔥 Burn<br/>(via Order / Payment API)"]
    C --> E["🔥 Burn<br/>(via Burn Coupon API)"]
    C --> F["↩️ Release<br/>Coupon (on failure)"]
...
→ coupon must be burned **manually** after payment.
...
but burned in System B (e.g., mobile → kiosk flow).
...
```mermaid
flowchart TB
    A["🏬 Customer presents FREE-MUG coupon"] --> B["🧪 Validate/Lock coupon (reserve)"]
    B --> C["🎁 Staff hands over free product"]
    C --> D["🔥 Burn Coupon API is called"]
    D --> E["✅ Coupon marked as Used"]
```
...
flowchart TB
    ...
    E -->|Yes| H["✅ Auto-burn happens in Track Order"]
    E -->|No| F

    G -->|Yes| I["✅ Auto-burn happens in Track Payment"]
    G -->|No| F
```
