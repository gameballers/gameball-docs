...
## **Summary: Widget vs Custom UI Integration**

This table shows what you need to implement for each approach:

| **Step**                   | **API Endpoint**                 | **Using Gameball Widget** | **Building Custom UI** |
| :------------------------- | :------------------------------- | :------------------------ | :--------------------- |
| 1. Show Redemption Options | `GET /redemption-configurations` | ✅ Handled by widget      | ⚙️ You must call API    |
| 2. Generate Coupon         | `POST /generate-coupon`          | ✅ Handled by widget      | ⚙️ You must call API    |
| 3. Display Coupons         | `GET /customers/id/coupons`      | ✅ Handled by widget      | ⚙️ You must call API    |
| 4. Validate & Lock         | `POST /validate-coupon(s)`       | ⚙️ You must implement     | ⚙️ You must implement   |
| 5. Burn/Apply Coupon       | `POST /burn-coupon` or `/order`  | ⚙️ You must implement     | ⚙️ You must implement   |
| 6. Release (if cancelled)  | `POST /release-coupon`           | ⚙️ You must implement     | ⚙️ You must implement   |

**Legend:**
- ✅ **Handled by widget** - Gameball automatically handles this step
- ⚙️ **You must implement** - You need to call the API in your code
