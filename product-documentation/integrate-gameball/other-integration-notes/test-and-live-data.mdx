...
## Shopify Stores: Test Data Limitation

<Warning>
Shopify stores always display **live data** by default — there is no built-in test environment on Shopify. Switching between live and test data modes in the Gameball dashboard is **not available for Shopify clients**.

If you need to test your Gameball setup before going live, the recommended approach is to create a separate **Shopify development store** and connect it to a separate Gameball workspace using your test API key. Note that Shopify dev stores do not support real credit card transactions, so you may need to contact Gameball support to arrange a plan for your test workspace.
</Warning>
