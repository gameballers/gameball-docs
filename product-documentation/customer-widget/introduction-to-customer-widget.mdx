...
<Accordion title="Points Section" icon="coins">
  ...
  If a customer has **pending points** — points earned but not yet activated — they appear alongside the balance with a clock icon. The pending points display is conditional: it only appears when the customer actually has pending points and is hidden when there are none.
</Accordion>
...
<Accordion title="Earn Tab" icon="star">
  ...
  2. **Badges Section**: Displays all active non-game reward campaigns — such as action-based or time-based challenges — along with progress indicators and completion status for each. Game campaigns (e.g., Spin the Wheel) are **not** shown here; they appear exclusively in the **Play & Win** tab.
</Accordion>
