...
<AccordionGroup>
  ...
  <Accordion title="How do you create multiple conditions under one segment?">
    ...
    You can add **unlimited rules** to a segment — there is no cap on the number of conditions.

    <Note>
    A single operator (AND or OR) applies uniformly across all rules in a segment. You cannot mix AND and OR within the same segment, and nested groupings with brackets (e.g., `(A AND B) OR C`) are not currently supported.
    </Note>
  </Accordion>
  ...
  <Accordion title="Can I filter customers by a date range, such as all customers created in 2025?">
    Date-based attributes like **Customer Creation Date** support **After** and **Before** operators, but not a single "between" operator.

    To define a date range, combine two rules using **AND** logic:

    - Rule 1: `Customer Creation Date` **After** `31/12/2024`
    - Rule 2: `Customer Creation Date` **Before** `01/01/2026`

    This returns all customers whose creation date falls within the year 2025.

    <Note>
    Quick filters by year, quarter, or month are not currently available. Use the After + Before combination to achieve the same result.
    </Note>
  </Accordion>
