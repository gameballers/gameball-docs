...
<Step title="Set the Streak Frequency">
  Choose how often customers need to act to maintain their streak:

  - **Daily** (default) — customers must perform the action each calendar day.
  - **Weekly** — customers must act at least once per week. Choose which day the week starts on; boundaries resolve in your account timezone.
  - **Monthly** — customers must act at least once per month. Milestones count streak steps, not calendar months.

  <Note>
  Frequency and week-start day lock once the campaign goes live and cannot be changed.
  </Note>
...
<Step title="Set Up the Rewards">
  ...
  <Warning>
  If the customer misses a day (or week, or month — depending on your chosen frequency), they start over. The streak resets, but milestone badges already earned remain with the customer.
  </Warning>
...
<Step title="Add Rewards and Badges">
  You can add up to **10 reward tiers**. Rewards should increase with longer streaks to encourage continued engagement.

  <Note>
  The 10-row limit refers to the maximum number of unique reward **tiers** (variations) you can configure — not a cap on how long the streak can run. Once the last tier is reached, its reward repeats for every subsequent period as long as the customer maintains their streak. Streaks continue indefinitely.
  </Note>
...
<Tab title="Web Visits">
  ...
  <Note>
  If you're not using a built-in Gameball plugin like **Shopify** or **Salla**, make sure the widget is integrated using our JavaScript SDK [guide](/installation-guides/v3/web/getting-started) to ensure proper tracking.
  </Note>
</Tab>
