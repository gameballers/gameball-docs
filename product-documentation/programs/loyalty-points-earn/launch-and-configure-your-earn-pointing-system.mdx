...
<Frame>
  <img src="/images/product-docs/programs/loyalty-points-earn/new-earn-page.png" alt="Points Earn configuration page" />
</Frame>
...
In the **Order Reward** section, click the **Place an order** card. A settings drawer will slide in from the right. Set:
...
The **Settings** section lets you configure how points are issued and which orders qualify. Click any settings card to open a drawer and update it.

<AccordionGroup>
<Accordion title="Points Expiry" icon="clock">
Define when earned points will expire. Click the **Points Expiry** card to open the settings drawer, then set the expiry duration and unit (days, months, or years).

<Frame>
  ![Points expiry settings drawer](/images/product-docs/programs/loyalty-points-earn/expiry-settings-points.png)
</Frame>

Points expiry is calculated from the exact moment points are earned. For example, with a 3-day expiry, points earned at 2:00 PM on January 1st expire at exactly 2:00 PM on January 4th (72 hours later).

**How expiry is enforced:**

- Customers **cannot redeem** expired points from the moment the expiry time passes — even before the system has processed the expiry.
- Expiry status is updated by a background job that runs **multiple times per day**. The dashboard may briefly show points as still active until the next job run, but redemption is blocked from the exact expiry moment.

<Note>
Points expiry and the return window interact: if a return window is configured, the expiry countdown starts only after points become active (i.e., after the return window ends), not from when they were earned.
</Note>
</Accordion>
