...
This guide helps you migrate from Gameball Flutter SDK v2 to v3.2.1.

## What's New in v3.2.1

- **Fixed — Widget Shop Parameter**: the `shop` value was sent to the widget URL under a duplicate `platform` key, which broke widget coupon redemption (infinite loading) when both `platform` and `shop` were configured. No API changes — update the version and run `flutter pub get`.
...
### Migrate v3.1.x → v3.2.1 (Non-Breaking)

- Update `pubspec.yaml` to `gameball_sdk: ^3.2.1`, then run `flutter pub get`.
...
Update your `pubspec.yaml` to use SDK v3.2.1:
