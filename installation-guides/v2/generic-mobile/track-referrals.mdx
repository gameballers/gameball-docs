---
title: "How Do I Track Referrals in a Generic Mobile App (v2, Legacy)?"
sidebarTitle: "Track Referrals"
...
Before you start, you must configure your Firebase on your Gameball account. Follow the steps in [Configure your Firebase account on Gameball for mobile friends referral links](/product-documentation/admin-settings/configure-firebase-push-notifications) article in our Help Center to configure your Firebase account on Gameball.
...
This referral link is generated dynamically and can be accessed via Gameball profile widget. It can also be accessed programmatically using the Gameball [Get Customer API](/api-reference/customers/management/get-customer) within its response as below snippet.
...
Once you have located the **GBReferral** parameter within a dynamic link. You will need to send its value and send along with the [Create Customer API](/api-reference/customers/management/create-customer) that is being executed for the newly created referred customer.

The below sample body for the [Create Customer API](/api-reference/customers/management/create-customer) creates a new customer with playerUniqueId {referred_player_id} that has been referred by the customer who owns the code of 001122 captured form the dynamic link.
