...
A coupon can carry up to four restrictions, each limiting where it may be used. The group a restriction belongs to decides how it behaves. Every coupon in the list is matched against the same cart, each using its own restrictions and its own operator.
...
A request that sends **no cart fields at all** is treated as an eligibility question — *"does this customer hold these coupons, and are they live?"* — rather than a cart question. Restrictions are reported back on each coupon instead of being enforced, so you can discover what to send next. Every other check (active, dates, audience, limits, combinability) still runs.
...
<Note>
`valid: true` on a cart-less call means *"this customer may use these coupons"*, **not** *"these coupons apply to this order"*. Before you complete the order, validate again with the cart described — that is the call that enforces `entitledMerchantIds`, `entitledBranchIds`, `entitledCollectionIds`, `entitledCategoryIds` and `entitledProductIds`, and it can reject a coupon that just came back valid.
</Note>
...
| `9017` | `totalPurchaseAmount` is below a coupon's `minOrderValue`. |
